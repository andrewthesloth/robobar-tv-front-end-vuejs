...
class RobobarOpcClient(Client):
    ...
    @staticmethod
    def get_items_from_circular_buffer(buffer, index_of_first_item, queue_length, index_of_buffer_start, index_of_buffer_end):
        ...
        Args:
            ...
            index_of_first_item (int): (aka readIndex) equals the index in buffer of first item on the stack (first to pop)
            queue_length (int): equals the number of currently pushed items
            index_of_buffer_start (int): equals the index of first buffer item (since PLC can start indexing from any integer) in the array
    ...
    def get_current_plc_time(self):
        ...
        return current_plc_time_string

    def get_datetime_dict_from_byte_array(self, byte_array):
        """Converts PLC's Date_and_Time variable to dictionary.

        Args:
            byte_array (List): defined in https://support.industry.siemens.com/cs/document/36479/date_and_time-format-for-s7-?dti=0&lc=en-WW
        ...
        for byte_index, dt_byte in enumerate(byte_array):
            ...
            low = dt_byte & 0xF;
            number = 10 * high + low;
            if self.VALUE_NAMES[byte_index] == 'year':
                number += 2000
            date_dict[self.VALUE_NAMES[byte_index]] = number
    ...
    def get_prep_drink_json(self, side=0):
        ...
        print(prep_drink_obj)
        return prep_drink_obj
...
if __name__ == "__main__":
    ...
    conn_thread.start()
    # test_client.create_and_maintain_connection()
